...
class Repository {
  constructor(git_directory, options) {
    this._git_directory = git_directory;
    this._options = options ? options : {};
    this._packs = [];
    this._loose = null;
    this._already_searched = {};
  }

  get git_directory() {
    return this._git_directory;
  }
  set git_directory(value) {
    this._git_directory = value;
  }
  ...
  get already_searched() {
    return this._already_searched;
  }
  set already_searched(value) {
    this._already_searched = value;
  }
...
/** Debug log a repository
/* @param {Object} options - Options
/* @param {Time} options.since - Time object specifying that you don't want commits BEFORE this
/* @param {Time} options.until - Time object specifying that you don't want commit AFTER this
/* @param {bool} options.first_parent - tells log to only walk first parent
/* @param {string|string[]} options.path_limiter - string or array of strings to limit path
/* @param {number} options.max_count - number to limit the output
*/
...
Repository.init = function(dir, bare, callback) {
  try {
    ...
    try { fs.statSync(dir + "/objects"); return callback(null, self) } catch(err) {};
